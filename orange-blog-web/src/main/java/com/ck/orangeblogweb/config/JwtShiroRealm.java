...
import com.ck.orangeblogdao.pojo.JwtToken;
...
public class JwtShiroRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        if(loginName != null){
            ...
            FndUserPo employee = null;
            if(!employeeExist){
                // 与数据库数据进行匹配校验
                employee = fndUserService.loginAccountByLoginName(loginName);
            }else{
                employee = JSON.parseObject(redisUtil.hget(principalCollection.toString(), LmEnum.USER_INFO.getName()).toString(), FndUserPo.class);
            }
            try {
                ...
                if(!permSetExist){
                    permissionSet = fndPermissionService.findAllPermissionsById(employee.getId());
